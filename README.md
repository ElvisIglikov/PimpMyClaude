...
```
У пунктов иконки (💰 💬 ⬇️ ⬆️ ▦ 👀 ⏬) и горячие клавиши, работают только когда окно Claude впереди: Обкэшить ⌘⇧N · Новый чат ⌘N (штатная) · Свернуть ⌘⌥↓ · Развернуть ⌘⌥↑ · Расставить ⌘⌥A · Показать ⌘⌥S · Прокрутить ⌘⌥D. Подсказки видны в самом меню; поменять — таблица `M.hotkeys` в начале файла. Lua-модуль шлёт команды двумя каналами
...
Только в приложении PimpMyClaude.app (Lua-модули тем не знают). Проверено живьём 03.09: четыре окна четырьмя темами.

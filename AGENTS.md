...
- **Кто этот чат (WF29, задачи #5448 и #5455)**: окно опознаётся id чата, а не заголовком — заголовок попапа это
  ...
  - **Покраска** — цели `ProjectPaint.targets()` берутся из ответов страниц, а не из заголовков окон. Ключ окна
    ...
    заголовку, как до WF29. Чат главного окна: свежий (младше 60 с) ответ страницы сильнее адреса из `status.json`.
...
- Живой `inject.js` у Элвиса **побайтно равен репозиторному `main`**, VERSION `wf20-g-2` (ручка свёрнутого поля,
  коммит `13333de`, давно в `main`). Ветка `wf29` поднимает VERSION до `wf29-a-1` — живой файл обновит гейт (`cp`).
...
`claude-patch/inject.js` — всегда ОДИН агент на волну (5100 строк, разделы 2а темы, 2б полоска, 12 «Обкэшить»,

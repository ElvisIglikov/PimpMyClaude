...
Обновлено 05.09.2026 21:05. Это файл-передача: новый чат открывает папку проекта и продолжает отсюда, ничего не
...
## Текущее (05.09 23:50 — марафон чата «Pimp» идёт; run-файл `docs/run-marathon-2026-09-05.json`, сторож `watchdog-pimpmyclaude-0509`)

- **Марафон в одном чате**: сделаны и слиты в main WF13 (новое окно), WF14 (меню «Оформление»), WF15 (цвет проекта),
  WF16 (новое окно в проекте), WF18 (живые цвета), WF19 (хвосты меню), WF24 (тесты в репозитории, `tools/test.sh`).
  Счёт в строке состояния — по этому марафону: было «WF 8 из 16 · идёт». Сводка `docs/status.md`, порядок и журнал —
  `docs/run-marathon-2026-09-04.json`, у каждого воркфлоу свой `docs/run-wfN-*.json` (status/next_steps/script_path/run_id).
- **WF20 «Оформление 2» — на гейте, ветка `wf20`** (не слита; приложение у Элвиса — сборка WF19). Сделано: меню слева от
  минуса, панель «Своя тема…» (три ручки), цвет проекта без действий (авто-цвет по имени папки, выбор в окне = тема проекта,
  тумблер «Цвет по проекту»), фикс-батч по verify, пункт переименован в «Вынести этот чат в окно». Осталось: батч D (доки),
  V2 (проверка), гейт — точные шаги в `docs/run-wf20-2026-09-04.json` → next_steps (resume по script_path/run_id).
- **Планы готовы (после критиков), Элвису не показаны**: WF25 своё меню-панель с анимацией + «🎨 Цвета ▸» для людей
  (дополнение 19:20 — максимальный приоритет Элвиса: сначала быстрой правкой NSMenu после WF20), WF26 автоперепатч,
  WF27 Sparkle-обновления, WF28 админ-панель чатов — `docs/plan-wf2[5-8]-2026-09-05.md`, макеты `docs/mockup-wf2[5-8].html`.
  Очередь после WF20: WF17 (релиз 1.4.0 — нужен Элвис: один из команды, «Поставить»), WF21 сетки (макет переделать
  под дополнения Элвиса в плане), WF22 полоска v3 (концепция `docs/PROGRESS.md` обязательна), WF23, затем 25→28.
...
- Живой `inject.js` у Элвиса = репозиторный `main` + ручка свёрнутого поля выше (коммит 13333de), VERSION `wf20-g-1`.
- Сторож `watchdog-pimpmyclaude-0409` снимет себя сам (все run-файлы не «идёт»); новому чату — поставить свой.

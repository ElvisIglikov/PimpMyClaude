...
(() => {
  ...
  // Вложения ПОЛЯ ВВОДА в узком окне (WF61, #5912). У Claude плитка вложения в
  // композере жёстко 120×120 (`[data-cds-tile-media]`, правило видно в ресурсах
  // app.asar), а в окне 280 на поле остаётся 194 точки — в ряд встаёт РОВНО
  // ОДНА плитка. Пять скриншотов = 600 точек, то есть всё окно: ни черновика, ни
  ...
  // Сторона плитки в узком окне. 56 + зазоры — это три штуки в ряд на 194
  // точках; меньше уже не картинка, а значок.
...
`;
  ...
  //
  // Правил два. Плитка: жёсткие 120×120 Claude меняем на 56×56, и в ряд встают
  // три, а не одна; `min-*: 0` — чтобы правило пережило день, когда Claude
  // допишет композеру свой `min-width` (в ленте у него он уже есть). Коробка:
  // потолок высоты и своя прокрутка, чтобы десяток вложений не съел окно.
...
@media (max-width: ${NARROW_WINDOW_MAX}px) {
  [data-cds-composer-attachments] [data-cds-tile-media] {
...
`;
  ...
  const progressWho = (role) => {
    ...
    const word = models.length === 0 || agents === null ? String(role.agents ?? "").trim() : "";
    const text = [word, list].filter(Boolean).join(" · ");
    return { text: text || "—", fable: models.some(item => item.fable) };
  };

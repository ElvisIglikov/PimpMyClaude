...
enum MenuModel {
    ...
    // MARK: - «Обкэшить» заменяет окно (#5768)

    /// Новое окно встало на место старого, старое закрыто. Отказ страницы («переносить было
    /// нечего») приложению не виден — поэтому говорим прямо, что осталось от старого окна.
    static let cashoutReplacedNotice = "Новое окно на месте старого. Старое закрыл — чат остался в списке слева"
    /// Новое окно за 40 с не открылось: старое не трогаем вовсе.

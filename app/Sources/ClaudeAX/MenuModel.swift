...
enum MenuModel {
    ...
    // Плашки «Экран мал для этой сетки — разложил лентой» больше нет: ⌥⌘A и «▦ Расставить»
    // с WF77 идут умным путём, где сетка считается по тому, как окна стоят, и в минимум
    // не упирается (критик WF77, блокер 3). Строку никто не звал — убрана на гейте волны 1.
    ...
    /// «🗂 PimpMyClaude» с папкой на значке читалось как «сейчас откроется папка» (слово Элвиса
    /// 17.09 16:10) — пункт зовётся темой и носит кружок своей палитры, как свои темы.
    static func projectThemeTitle(_ name: String) -> String { "Тема проекта \(name)" }

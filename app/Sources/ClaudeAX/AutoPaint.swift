...
enum AutoPaint {
    ...
    /// Индекс схемы на запуск: у «Случайно» — новый случайный, у остальных наборов схема своя
    /// всегда — nil. `repeating` — повторить заданную (так делал «🔁 Ещё раз» до WF19),
    /// `avoiding` — выбрать любую, КРОМЕ заданной: гармоний всего четыре, и честный случайный
    /// выбор в четверти случаев вернул бы ту же самую (критик В3 плана WF19).
    ///
    /// Случайность — параметром (`pick` получает размер выбора и возвращает индекс в нём):
    /// иначе поведение «Ещё раз» и «Случайно» тестами не проверить (критик Б3).
    static func schemeIndex(for preset: AutoPaintPreset, repeating: Int? = nil,
        ...
        if let repeating = repeating { return clampScheme(repeating) }
        if let avoiding = avoiding.map(clampScheme), randomSchemes.count > 1 {
            // Выбор из трёх оставшихся: индекс, попавший в прежний или дальше, сдвигаем на один.
            let index = min(max(pick(randomSchemes.count - 1), 0), randomSchemes.count - 2)
            return index < avoiding ? index : index + 1
        }
        return clampScheme(pick(randomSchemes.count))
    }

    /// Индекс схемы в границах каталога гармоний: в памяти может лежать что угодно.
    static func clampScheme(_ index: Int) -> Int { min(max(index, 0), randomSchemes.count - 1) }

    /// Тёмная или светлая у «🎲 Случайно» (решение 3 плана WF19): каждый раз наоборот к прошлой
    /// покраске, а прошлой нет — монетка. Считать по галкам окон нельзя: покраска их сама
    /// снимает, и «Случайно» после неё всегда выпадало тёмным.
    static func nextLight(last: Bool?, coin: () -> Bool = { Bool.random() }) -> Bool {
        last.map { !$0 } ?? coin()
    }
...
/// выбираются на запуск. Режим нужен «Ещё разу» (он его не меняет) и самому «Случайно»:
/// по нему считается следующий, наоборот к прошлому (решение 3 плана WF19). Гармония после
/// WF19 у «Ещё раза» новая — прежняя лежит здесь ради того, чтобы её НЕ повторить.
